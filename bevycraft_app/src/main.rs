...
const MAX_GARBAGE_LT: f64 = 60.0f64;

fn main() -> AppExit {
    App::new()
        .add_plugins(DefaultPlugins)
        .insert_resource(SectionPool::new(MAX_GARBAGE_LT))
        .insert_resource(Time::<Fixed>::from_hz(64.0))
        .insert_resource(RModelManager::default())
        .init_state::<AppState>()
        .add_systems(
            OnEnter(AppState::RegisteringContent),
            init_registries
        )
        .add_systems(
            OnEnter(AppState::LoadingRModels),
            load_block_models
        )
        .add_systems(
            OnEnter(AppState::LoadingTextures),
            load_textures_into_server
        )
        .add_systems(Update, wait_for_server
            .run_if(in_state(AppState::LoadingTextures))
        )
        .add_systems(
            OnEnter(AppState::BuildingArrayTexture),
            build_array_texture)
        .add_systems(
            OnEnter(AppState::SolvingBlockModels),
            solve_models
        )
        .run()
}
